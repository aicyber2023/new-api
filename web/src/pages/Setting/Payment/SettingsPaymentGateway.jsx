...
export default function SettingsPaymentGateway(props) {
  ...
  useEffect(() => {
    if (props.options && formApiRef.current) {
      const currentInputs = {
        ...
        AlipaySandbox: props.options.AlipaySandbox === 'true',
      };
  ...
  const submitPayAddress = async () => {
    ...
    try {
      ...
      if (originInputs['AlipayPublicKey'] !== inputs.AlipayPublicKey) {
        options.push({ key: 'AlipayPublicKey', value: inputs.AlipayPublicKey });
      }
      if (originInputs['AlipaySandbox'] !== inputs.AlipaySandbox) {
        options.push({ key: 'AlipaySandbox', value: inputs.AlipaySandbox.toString() });
      }
          ...
          <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 24, xl: 24, xxl: 24 }} style={{ marginTop: 16 }}>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Switch
                field='AlipaySandbox'
                label={t('沙箱模式')}
                extraText={t('开启后使用支付宝沙箱环境，仅用于测试')}
              />
            </Col>
          </Row>
